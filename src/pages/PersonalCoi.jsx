    ...
    return (
        <div className="card">
            ...
            <div style={{ padding: 20, background: 'rgba(72, 184, 208, 0.05)', borderRadius: 12, marginBottom: 24, display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                <CornerUpLeft className="text-secondary-color" size={24} style={{ marginTop: 2, flexShrink: 0 }} />
                <div>
                ...
                <table className="table">
                    ...
                    <tbody>
                        {declarations.map((dec, idx) => (
                            <tr key={idx}>
                                <td data-label="対象年度" style={{ fontWeight: 600 }}>{dec.year}</td>
                                <td data-label="申告区分">{dec.type}</td>
                                <td data-label="合計受領額">{dec.amount}</td>
                                <td data-label="提出日">{dec.submittedAt}</td>
                                <td data-label="ステータス">
